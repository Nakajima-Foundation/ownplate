const data = {
  serviceName: {
    ownPlate: "OwnPlate",
    omochikaeri: "おもちかえり.com",
  },
  pageTitle: {
    restaurantRoot: "Liste des restaurants qui font des plats à emportés",
    restaurantArea:
      "Liste des restaurants qui font des plats à emportés dans la  {area}",
    terms: {
      admin: "CGU pour les propriétaires de restaurants",
      user: "CGU pour les utilisateurs",
      privacy: "Politique de Confidentialité",
    },
  },
  adminTitle: {
    menuList: "Éléments du Menu",
    orderList: "Liste des Commandes",
    orderHistory: "Historique de Toutes les Commandes",
    orderDetail: "Détails de la Commande",
    orderSuspend: "Paramètres de Suspension des Commandes",
    autoPrinter: "Impression Automatique",
    lineAuth: "Paramètres d'Intégration LINE",
    discountList: "Liste des Paramètres de Réduction",
    discountDetail: "Paramètres de Réduction",
    discountHistory: "Historique des Réductions",
  },
  underConstruction:
    "ATTENTION: Ceci est un serveur de préproduction pour le dévelopement.",
  comma: ", ",
  mobileOrder: {
    name: "Online ordering service",
    closing: {
      notice: "Avis",
      heading: "Avis de fin de service",
      paragraph1:
        "Nous vous remercions pour votre soutien continu. La commande mobile ne sera plus disponible le vendredi 18 août 2023. Nous nous excusons pour les désagréments occasionnés et vous remercions de votre compréhension.",
      paragraph2:
        "Vous trouverez ci-dessous l'horaire jusqu'à la fin du service.",
      paragraph3: "Les nouvelles commandes ne sont plus acceptées.",
      ordering: "[Date et heure de la commande]",
      orderingDate: "Jusqu'à 16 heures, le jeudi 10 août 2023",
      acceptance: "[Date et heure de réception]",
      acceptanceDate: "Jusqu'à 22 heures, le vendredi 18 août 2023",
      closedMessage: "Avis de fin de service",
      closedParagraph:
        "Nous vous remercions pour votre soutien continu. La commande mobile ne sera plus disponible le vendredi 18 août 2023. Nous nous excusons pour les désagréments occasionnés et vous remercions de votre compréhension.",
    },
  },
  profile: {
    title: "Profile",
    address: "Personal Info",
    resetAddress: "Reset info",
    resetAddressMessage: "Reset personal info",
    resetAddressModify:
      "You can change your personal info when ordering a delivery",
    loginStatus: " Se Connecter Status",
    displayName: "Nom",
    lineConnection: "Connnection LINE ",
    lineFriend: "Ami avec le compte officiel",
    undefined: "Non défini",
    friendLink: "Devenir Ami",
    signIn: "Se connecter comme utilisateur",
    signInRestaurant: "Se connecter comme Restaurant",
    deleteAccount: "Supprimer le compte",
    reallyDeleteAccount:
      "Voullez vous vraiment supprimer cet compte? Tous vos données seront supprimés.",
    reSendSMSforDeleteAccount:
      "Send an SMS verification code to your registered phone number for final confirmation.",
    stripeInfo: "Carte de crédit associée",
    noCard: "Non enregistré",
    deleteCard: "Supprimer les informations de la carte",
    reallyDeleteCard:
      "Voullez vous vraiment supprimer les informations de la carte？",
    status: {
      hasLine: "Connecter",
      noLine: "Déconnecter",
      verifying: "Vérification en cours...",
      email: "email",
      phone: "Télephone",
      line: "LINE",
      unexpected: "Inattendu",
      isFriend: "un ami",
      noFriend: "pas un ami",
      none: "Aucun",
    },
  },
  menu: {
    about: "A Propos",
    admins: "Propriétaires de Restaurant",
    adminManual: "Manuel pour les Propriétaires",
    adminSupport: "Aides pour les Propriétaires",
    adminUserGroup: "Groupe des Propriétaires",
    users: "Amoureux de la nourriture",
    userManual: "Manuel pour les Amoureux de la nourriture",
    userSupport: "Aides pour les Amoureux de la nourriture",
    home: "Acceuil",
    signOut: "Se Déconecter",
    forRestaurantOwner: "Pour le restaurant",
    forCustomer: "Pour l'utilisateur",
    forAllUser: "Pour tous les utilisateurs",
    manualLink: "Anual (en japonais)",
    deliveryManualLink: "Delivery manual",
    printerManualLink: "Printer manual",
    tipsLink: "Conseils (en japonais)",
    termsRestaurant: "CGU pour le restaurant",
    termsUser: "CGU pour l'utilisateur",
    privacy: "Politique de Confidentialité",
    clickHereToOrder: "Click here to order",
    faq: "FAQ",
    updated: "Dernière Mise à jour",
    selectLanguage: "Selectioner la langue",
    yes: "Oui",
    no: "Non",
    close: "Fermer",
  },
  restaurantId: {
    self: "Identifiant du Restaurant ",
    available: "Cet identifiant de restaurant est disponible",
    submit: "S'inscrire",
    placeholder: "Veuillez indiquer un identifiant de restaurant unique",
    tooshort:
      "L'identifiant du restaurant doit comporter au moins cinq caractères.",
    invalid:
      "Identifiant invalide. Vous ne pouvez utiliser que des chiffres et des nombres.",
    alreadyTaken: "Cet identifiant n'est pas disponible.",
  },
  restaurantCsv: {
    date: "date",
    restaurantName: "restaurant name",
    onTheList: "on the list",
    publicFlag: "is public",
    deletedFlag: "is deleted",
    menu: "number of menu",
    uid: "uid",
  },
  editCommon: {
    save: "Enregistrer",
    saving: "Enregistrement",
    saveDraft: "Enregistrer comme brouillon",
    new: "Nouveau",
    copy: "Copier ce restaurant",
    copyAlert:
      "Copiez ce restaurant et créez-le en tant que nouveau restaurant privé.",
    copyMenu: "Copiez cet élément dans le restaurant suivant",
    copyMenuAlert:
      "Copiez cet élément dans le restaurant en tant que nouvel élément privé.",
    current: "Actuel",
    clickAndUpload: "Cliquer et Télécharger",
    clickAndUploadDetail:
      "Cliquez sur la zone grise ou Glissez-déposez pour télécharger l'image (JPEG uniquement)",
    edit: "Éditer",
  },
  currency: {
    USD: "USD",
    JPY: "JPY",
    EUR: "EUR",
  },
  tax: {
    price: "{count}",
    include: " (inc. tax)",
    exclude: " (+ tax)",
    taxExample:
      "Si le prix est défini comme {count} dans les paramètres, il sera affiché comme : ",
  },
  editRestaurant: {
    enterRestaurantName: "Entrer le nom d'un restaurant",
    enterOwnerName: "Entrer le nom du propriétaire du restaurant",
    enterStreetAddress: "Enter l'adresse de la rue (ex: Marunouchi 1-9-1)",
    enterCity: "Enter Ville (ex: Chiyoda-ku)",
    enterZip: "Enter Code Postal",
    enterPhone: "Enter numéro de Téléphone",
    enterWebsite: "Enter lien du site Web",
    enterLineUrl: "Enter LINE Official Account URL",
    enterInstagramUrl: "Enter Instagram URL",
    enterUberEatsUrl: "Enter UberEats URL",
    enterIntroduction: "Entrez une présentation de la boutique",
    enterOrderNotice: "Entrez les notifications de commande",
    enterOrderThanks: "Entrez le message de remerciement de la commande",
    enablePrelineTitle: "Enable line breaks",
    enablePrelineDescription:
      "Enable line breaks for the introduction, order notice, order thanks of  the restaurant and item description of menu",
    introduction: "Présentation de la boutique",
    orderNotice: "Notifications de commande",
    orderThanks: "Mot de remerciement de la commande",
    website: "Site Web",
    tax: " Taxe de vente",
    foodTax: "Taxe sur nouriture",
    alcoholTax: "Taxe boisson",
    taxPriceDisplay: "Calcul de la taxe",
    taxPriceDisplayJp: "L'étiquetage avec taxe est obligatoire au Japon.",
    taxIncluded: "Taxe Inclue",
    taxExcluded: "Taxe Exclue",
    about: "A Propos",
    required: "Requis",
    personalInfo: "Méthode de Sauvegarde des Noms des Clients",
    paymentMethods: "Payment methods",
    timeToPickup: "Temps de ramassage",
    preparationTime: "Temps de préparation du plat emporter",
    preparationTimeDescription: "Shortest time from order to takeaway",
    deliveryPreparationTime: "Delivery preparation time",
    reservationTheDayBefore: "Réservation la veille",
    reservationTheDayBeforeDescription: "When do you accept orders",
    reservationTheDaysBefore: {
      zero: "Seulement le jour",
      one: "A partir de 1 jour à l'avance",
      two: "A partir de 2 jour à l'avance",
      three: "A partir de 3 jour à l'avance",
      four: "A partir de 4 jour à l'avance",
      five: "A partir de 5 jour à l'avance",
      six: "A partir de 6 jour à l'avance",
      seven: "A partir de 7 jour à l'avance",
      twoWeeks: "From two weeks in advance",
      threeWeeks: "From three weeks in advance",
      twoMonths: "From two months in advance",
      threeMonths: "From three months in advance",
    },
    minimumCookTime: {
      halfHour: "30 minutes",
      oneHour: "One hour",
      twoHours: "Two hours",
      oneDay: "One day",
      twoDays: "Two days",
      threeDays: "Three days",
      fourDays: "Four days",
      fiveDays: "Five days",
      sixDays: "Six days",
    },
    minutes: "minutes",
    withinfive: "Dans les 5 heures",
    withinaday: "Dans les 24 heures",
    withinFiveDays: "Within 6 days",
    personalInfoSaveMethodChoices: {
      notRequired: "Ne Pas Sauvegarder",
      optional: "Sauvegarder le Nom S'il Est Fournit",
      required: "Sauvegarder Comme Requis",
    },
    paymentMethodChoices: {
      cache: "Cache",
      suica: "Suica/Pasmo",
      creditCard: "Credit Card",
      paypay: "PayPay",
      rpay: "Rakuten Pay",
      dpay: "d pay",
      id: "iD",
      quick: "QUICPay",
      applepay: "Apple Pay",
      line: "LINE Pay",
      merpay: "Merpay",
      aupay: "au PAY",
      alipay: "Alipay",
      wechatpay: "WeChat Pay",
      unipay: "UnionPay",
    },
    profilePhoto: "Photo de Profil du Restaurant",
    coverPhoto: "Photo de Couverture du Restaurant",
    setupMap: "Restaurant location",
    setLocation: "Définir l'emplacement",
    searchMap: "Set the location from the address",
    enterAddress: "Veuillez entrer une adresse pour la recherche",
    updateMap: "Mettre à jour la carte",
    businessHourDescription:
      "Vérifiez le jour ouvrable de la semaine et définissez les heures d'ouverture. Si vous devez fractionner vos heures d'ouverture, comme une pause, veuillez saisir cette dernière heure dans le formulaire Autres heures.",
    businessHourOption: "Autres heures. (facultatif)",
    copyPreviousDay: "Copier le jour précédent",
    copySunDay: "Copier le jour précédent",
    lastOrderTime: {
      title: "Heure limite de commande",
      notes1:
        "Configurez cette option si vous souhaitez clôturer les commandes avant « l'heure de commande finale », en tenant compte de « l'heure de fermeture de l'entreprise » et du « temps de préparation de la commande ».",
      notes2:
        "Si cette option n'est pas configurée, l'heure sera déterminée en tenant compte de « l'heure de fermeture de l'entreprise » et du « temps de préparation de la commande ».",
    },
    draftDescription: "Enregistrer comme brouillon.",
    publishDescription: "Enregistrez et publiez votre restaurant.",
    draftWarning: "Vous pouvez publier ceci en corrigeant le cadre rouge.",
    updateMapDescription:
      "Définir l'emplacement du restaurant à partir du nom et de l'adresse de votre restaurant.",
    noRestaurant: "Aucun nom de restaurant",
    noStreetAddress: "Aucun adresse de la rue",
    reallyDelete: "Voulez-vous vraiment supprimer ceci?",
    reallyOnListDelete: "Do you really want to delete this from lists?",
    deliveryConfigTitle: "Delivery",
    deliveryDescription:
      "You can manage your own delivery. By setting the delivery area etc., the customer can choose takeout or delivery when ordering. (Don't forget to save if you have changed the store settings)",
    deliveryConfigLink: "To delivery settings",
    printerConfigTitle: "Printer settings",
    enablePrinter: "Activer l'impression",
    printerDescription:
      "You can print your order information using a printer made by Star Seiki connected via Bluetooth.",
    printerDescription2:
      "If enabled, a print button will be displayed on the order details page after accepting the order",
    printerDescriptionConfig: "Click here for automatic print settings",
    phoneCall: "Phone notification",
    phoneCallDescription: "Make a call when a new order comes in",
    phoneCallNotice:
      "Although it is currently provided free of charge, this feature may be a paid feature in the future due to telephone call charges. In that case, even if this function is turned on, there is no sudden cost",
    phoneCallNotification: "Phone Notification",
    emailNotification: "Notification par email",
    emailNotificationTitle: "Notification par email",
    emailNotificationDescription:
      "Envoyer un courriel lorsqu'une nouvelle commande arrive",
    emailNotificationNotice:
      "Si vous ne le trouvez pas dans votre boîte de réception, veuillez vérifier votre dossier de courrier indésirable.",
    acceptUserMessage: "Accept a message with customer",
    acceptUserMessageDescription: "Accept a message with customer",
    acceptUserMessageNotice:
      "Allows customers to enter a single message on the order confirmation screen.",

    lineNotification: "LINE Notification",
    city: "Vile",
    zip: "Code Postal",
    state: "Région",
  },
  editMenu: {
    menu: "Menu",
    enterItemName: "Entrez le nom de l'article",
    enterItemAliasesName:
      "Veuillez entrer un alias pour le produit (par exemple, une langue étrangère).",
    enterItemDescription: "Entrez la description de l'article",
    enterItemMemo:
      "Memo (Memo will not be displayed on customer's page. Please do not write confidential information)",
    enterItemOption: "Entrez l'option d'article",
    item: "Article",
    noItem: "Aucun Article",
    pleaseAddItem: "Veuillez créer votre menu à partir du bouton du bas.",
    required: "Requis",
    itemName: "Nom de l'article",
    itemAliasesName:
      "Alias de l'élément (Cette information n'est affichée que dans la page d'administration).",
    price: "Prix",
    free: "Free",
    optionsPreview: "Aperçu des options",
    priceChange: "Changement de prix",
    noPriceChange: "Pas de changement",
    tax: "Taxe",
    food: "Nouriture",
    alcohol: "Alcool",
    foodJP: "Taux de taxe réduit sur les aliments",
    alcoholJP: "Tout autre chose comme l'alcool, les marchandises",
    displayPrice: "Exemple de prix",
    itemDescription: "Description de l'article",
    itemMemo: "Memo",
    itemPhoto: "Photo de l'article",
    additionalPhotos: "Photos supplémentaires (jusqu'à 4 photos)",
    addPhotos: "Sélectionnez les photos",
    deleteAllPhotos: "Supprimer toutes les photos",
    itemOptions: "Option de l'article",
    itemOptionsNote:
      'Pour les alternatives, utilisez une virgule pour les séparer (ex: "Poulet, Boeuf, Mouton "). Vous pouvez ajouter un prix à une option avec (+ N), où N est le prix.',
    itemAddOption: "Ajouter une option",
    itemInvalidMessage:
      "Cet élément ne figure pas dans votre menu car les données ne sont pas valides.",
    saveAsDraft:
      "Enregistrer comme brouillon. Cet élément ne figure pas dans vos menus.",
    reallyDelete: "Voulez-vous vraiment supprimer cela?",
    csvCategories: "Paramètres de catégorie pour le fichier Excel (facultatif)",
    csvCategoriesNote:
      "Il s'agit d'un paramétrage de catégorie à utiliser dans le fichier Excel qui peut être téléchargé à partir de l'historique des commandes.",
    newCategory: "Nouveau point",
    newCategoryAdd: "Ajouter",
    category1: "Catégorie 1",
    editCategory1: "Modifier Catégorie 1",
    category2: "Catégorie 2",
    editCategory2: "Modifier Catégorie 2",
    exclusionDate: "Exclusion Date",
    exclusionTime: "Exclusion Time",
    showAllMenu: "Tous les points",
    showPublicMenu: "Produits publiés",
    showSoldOutMenu: "Sold Out Items",
    selectCategory: "Sélectionner",
  },
  editTitle: {
    enterCategory: "Entrez le nom de le groupe",
    empty: "(Nom du groupe vide)",
  },
  find: {
    area: "Allons rechercher",
    areaAll: "Restaurants nationaux",
    areaTop: "Allons rechercher",
    likes: "Aime",
    favoriteShop: "Favoris",
    noLikes: "Pas de restaurants préférés",
    nofavoriteShop: "No Favorite Stores",
    allRestaurants: "Trouver des restaurants",
    shopList: "Liste des magasins",
    areas: {
      hokkaido: "Hokkaido Area",
      kanto: "Kanto Area",
      chubu: "Chubu Area",
      hokuriku: "Hokuriku Area",
      kansai: "Kansai Area",
      chugoku: "Chugoku Shikoku Area",
      kyusyu: "kyusyu Area",
      USA: "USA",
    },
  },
  shopInfo: {
    thisIsPreview: "Ceci est un aperçu uniquement.",
    notPublic: "La page n'est pas publique pour les utilisateurs.",
    share: "Partager",
    like: "Aimer",
    liked: "Aimée",
    visitWebsite: "Visiter le site web",
    qrcode: "Code QR",
    copyUrl: "Copier le Lien",
    copyMenuUrl: "Copier le Lien",
    UrlCopied: "Lien Copié",
    UrlCopyFailed: "Copie du Lien a échoué",
    name: "Nom du Restaurant",
    ownerName: "Nom du propriétaire du restaurant",
    restaurantDetails: "Details du Restaurant",
    streetAddress: "Adresse de la rue (ex: Marunouchi 1-9-1)",
    city: "Ville (ex: Chiyoda-ku)",
    state: "Région",
    prefecture: "Préfecture",
    zip: "Code Postal",
    address: "Adresse",
    phonenumber: "Téléphone",
    website: "Site web",
    lineUrl: "LINE Official Account URL",
    instagramUrl: "Instagram URL",
    uberEatsUrl: "Uber Eats URL",
    hours: "Heures",
    midnight: "12:00 minuit",
    noon: "12:00 midi",
    am: "{formatedTime} Matin",
    pm: "{formatedTime} Après-midi",
    howToReceive: "How to receive",
    closed: "Fermer",
    public: "Publique",
    temporaryClosure: "Temporairement Fermé",
    temporaryClosureAdd: "Ajouter",
    temporaryClosureDescription:
      "Veuillez sélectionner une date et appuyer sur Ajouter.",
    temporaryClosureOkay: "D'accord",
    temporaryClosureSelect: "Sélectionnez une date",
    paymentMethod: "Methode de Payment",
    onlinePayment: "Payment en Ligne",
    onsitePayment: "Paiement à la Livraison",
    noPaymentMethod: "Commande en ligne indisponible",
    notPublicShopMessage: "Not Public",
    notPublicShop: "Not Public",
    paymentMethods: "Payment methods",
    minimumAvailableTime: "Temps disponible minimum",
    lastOrder: "本日最終受付",
    todayNotAvailable: "翌日以降の受け取りとなります",
    noAvailableTime: "Aucun de temps de commande",
    deliveryThresholdNotice:
      "Delivery is possible from orders totaling {price} or more.",
    deliveryArea: "Delivery area",
    deliveryAreaInfo: "Details will be displayed on the next order page.",
    deliveryFeeInfo: "The delivery fee will be {price}.",
    deliveryFeeThresholdInfo: "Free shipping for {price} and above",
    deliveryAreaRadius: "Approximately within {radius} m from the restaurant",
    buttonDeliveryFeeThreshold: "Can be delivered for {price} JPY or more",
    buttonDeliveryFeeDiff: "And {price}",
    buttonDeliveryFee: "Delivery fee: {price}",
    takeout: "Takeaway",
    delivery: "Delivery",
    takeoutDescription: "Pick up at shop",
    deliveryDescription: "Delivery to home or work",
    lunchOrDinner: "Déjeuner/Dîner",
    lunch: "Déjeuner",
    dinner: "Dîner",
    viewMore: "Voir plus",
    viewLess: "Proche",
    productCategory: "Catégories de produits",
  },
  transactionsAct: {
    title: "Loi sur les transactions commerciales spécifiées",
    sellerName: "Nom du Vendeur",
    representative: "Représentant(e)",
    address: "Adresse",
    hours: "Heures",
    phone: "Téléphone",
    price: "Pix de vente",
    priceDescription: "Veuillez consulter la page du produit.",
    otherFees: "Autres dépenses",
    otherFeesDescription:
      "Vous êtes responsable de toutes les dépenses liées au transport vers et depuis le magasin.",
    payment: "Heure et mode de paiement",
    paymentDescription: "Paiement par carte de crédit / paiement en magasin",
    paymentDescriptionCard: "Paiement par carte de crédit",
    paymentDescriptionStore: "Paiement en magasin",
    paymentDescriptionCardNote:
      "If an error occurs while payment procedures even though you selected online payment, we are sorry but please pay at the store.",
    delivery: "Heure et mode de livraison",
    deliveryDescription:
      "Date spécifiée par le client et ramassage en magasin.",
    cancellation: "Annulation",
    cancellationDescription1:
      "Vous pouvez annuler votre commande sur Internet avant que l'état de la commande ne soit passé commande; Une fois l'état changé, veuillez contacter le magasin.",
    cancellationDescription2:
      "In the following cases, we may cancel the transaction related to your order and you are required to pay the amount equivalent to that price as a general rule.",
    cancellationDescription3:
      "In addition to the above, even accepted orders may be canceled due to material shortages, sell-outs, excessive orders, errors in order acceptance, or other reasons at the discretion of the member store. If you have already paid for the order, you may request a refund from the store, but you may not seek compensation for damages beyond the refund unless the store has committed intentional or gross negligence.",
    cancellationDescription4:
      "The order cannot be canceled after the Accepted message is displayed.",
    cancellationDescription5:
      "We may cancel the transaction related to your order.",
    cancellationDescription6: "for details.",
    takeoutTitle: "Takeaway",
    cancellationTakeoutDescription1:
      "When 2 hours have elapsed after the Ready to Pickup message is displayed.",
    deliveryTitle: "Delivery",
    cancellationDeliveryDescription1:
      "If more than 5 minutes have passed since you arrived at the designated delivery location and you are unable to contact us.",
    cancellationDeliveryDescription2:
      "If more than 15 minutes have passed since arriving at the designated delivery location",
    cancellationDeliveryDescription3:
      "When a place that violates the provisions of these Terms is specified as a delivery place",
    link: "Please click here",
  },
  sitemenu: {
    menu: "Menu",
    about: "A Propos",
    limitedSale: "Limited sale on the day of the week",
    unavailableTime: "Unavailable Time",
    confirmCart: "Confirm Cart",
    checkout: "Payement",
    orderCounter: "{count} article | {count} artciles",
    add: "Ajouter",
    soldOut: "Complet",
    soldOutToday: "Épuisé aujourd'hui",
    pickupAvailableAfterToday: "Retrait disponible à partir de demain.",
    options: "Options",
    subTotal: "Sous-total: ",
    quantity: "Quantité",
    addDifferentOptionsItem: "Add Different Options Item",
    openCollapse: "Voir d'autres images et options",
  },
  date: {
    today: "Aujourd'hui",
  },
  lp: {
    enjoyOmochilaeri: "Profitez de notre service !",
    operationTeams: "Équipes d'intervention",
    easy3Steps: "COMMENT COMMENCER？ 3 ÉTAPES FACILES !",
    forOwner: "Pour les propriétaires de restaurant",
    moreDetails: "Plus de détails",
    demoVideo: "Demo video",
    orderFlow: "Order Flow",
    flyerCreation: "Flyer Creation",
    cardPayment: "Card Payment",
    playVideo: "Play video",
    tryService: "Essayez le service Omochikaeri",
    ask: "Nous contacter",
    whatsnew: "Quoi de neuf ?",
    forCustomer: "Pour les clients",
    customerVoice1: "Voix des clients",
    customerVoice2: "",
    goToSampleRestaurant: "Aller au restaurant d'échantillons",
    tokyo: "Tokyo",
    osaka: "Osaka",
    izakaya: "Izakaya",
    patisserie: "Pâtisserie",
    searchRestaurant: "Rechercher un restaurant",
    forRestaurantOwner: "Pour les restaurateurs",
    signUpForFree: "Ouvrir un magasin gratuitement",
    clickHereToOrder: "Cliquez ici pour commander des plats à emporter",
    feature: "Caractéristiques de Omochikaeri.com",
    feature1: "Paiement par carte en ligne",
    feature2: "Livraison en interne",
    feature3: "Support d'impression avec un imprimeur dédié",
    feature4: "Lien d'inventaire avec Smaregi",
    howToStart: "Comment commencer",
    step1: "Créer un compte",
    step2:
      "Enregistrer les informations relatives à votre magasin et à votre menu",
    step3: "Enregistrer le paiement et les autres paramètres",
    step4: "Commencez le service de vente à emporter !",
    clickHereForDetails: "Cliquez ici pour des instructions détaillées",
    enjoy: "Profitez de notre",
    omochikaeri: "service!",
    forOwnerToOpen: "Pour les restaurants souhaitant ouvrir un magasin",
    clickHereToSignup: "Cliquez ici pour vous inscrire gratuitement",
  },
  promotion: {
    heading: "Campagnes en cours",
    period: "Période:",
    terms: "Conditions d'utilisation:",
    applied: "est le montant minimum de la réduction appliquée",
    oneTimeOnly: "Disponible pour une utilisation unique",
    noLimit: "Disponible autant de fois que vous le souhaitez",
    fromAmountSpent: "",
    discount: "de réduction sur le montant dépensé",
    notes: "Notes sur l'utilisation",
    notes1:
      "La remise promotionnelle sera appliquée automatiquement lorsque le montant dépasse le montant indiqué dans les conditions d'utilisation. Veuillez vérifier le montant total sur l'écran de confirmation de la commande après avoir cliqué sur le bouton de paiement.",
    notes2:
      "Une seule promotion est disponible par commande, c'est donc la promotion avec la remise la plus importante qui sera appliquée.",
  },
  order: {
    allOrders: "Toutes les commandes",
    menu: "Afficher la page du menu",
    orderNotPlacedYet:
      "La commande n'a pas encore été passée. Veuillez confirmer la commande et appuyer sur le bouton Passer la commande en bas.",
    yourOrder: "Votre commande",
    confirmOrder: "Confirmer votre commande",
    editItems: "Modifier les artciles",
    revenue: "revenu",
    foodRevenue: "food revenue",
    alcoholRevenue: "alcohol revenue",
    date: "date",
    type: "type",
    orderTypeEC: "EC",
    orderTypeDelivery: "delivery",
    orderTypePickup: "pickup",
    orderTypePreOrder: "pre order",
    orderTypeTakeout: "takeaway",
    tipShort: "Pourboire",
    serviceTax: "tip tax",
    name: "order name",
    restaurantName: "Nom du restaurant",
    payment: "payment",
    paymentCancel: "payment cancel",
    includingTip: "avec pourboire",
    subtotal: "Sous-Total",
    salesTax: "Taxe de vente",
    foodTax: "Taxe alimentaire",
    inclusiveTax: "Taxe de vente (compris)",
    total: "Total",
    requiredUserName: "Nom (Obligatoire)",
    enterUserName: "Veuillez entrer votre nom",
    yourPayment: "Votre Payment",
    details: "Details",
    print: "Print",
    thisIsDeliveryOrder: "This is a delivery order",
    thisIsTakeoutOrder: "This is a takeaway order",
    alertReqireAddress: "The input is not completed. Please check.",
    saveAddress: "Save delivery address",
    order_canceled_by_customer: "Canceled by customer",
    order_canceled_by_restaurant: "Canceled by restaurant",
    reuseCard: "Conservez-le pour une utilisation future",
    whatsCVC: "C'est quoi le CVC?",
    card_error: "Échec de la confirmation du paiement.",
    no_jcb: "＊Ce restaurant n'accepte pas la carte JCB",
    "3digitsCVC": "Les trois derniers chiffres au dos",
    "4digitsCVC": "Les quatre chiffres sur le devant",
    placeOrder: "Passer la commande - Payer maintenant",
    or: "OU",
    placeOrderNoPayment: "Passer la commande - payer plus tard",
    placeOrderNoPaymentMo: "Passer la commande - payer plus tard",
    placeOrderNoPaymentNote: "Payer lorsque vous recevez les articles",
    waitingPayment: "Waiting Payment",
    submitPayment: "Soumettre le paiement",
    validating: "Traitement en cours...",
    thankyou: "Merci!",
    orderStatus: "Statut de la commande",
    orderId: "ID de la commande",
    cancelOrder: "Annuler la commande",
    cancelOrderConfirm: "Voulez-vous vraiment annuler cette commande?",
    cancelOrderComplete: "La commande a été annulée",
    pleaseStay:
      "Veuillez rester sur cette page. Le statut de la commande sera mis à jour ci-dessous.",
    thanksMessage: "Message du restaurant",
    tip: "Pourboire",
    maxTip: "Montant jusqu'à {max}% (facultatif)",
    totalCharge: "Facture Total",
    pleasePayAtRestaurant:
      "Le paiement en ligne n'a pas encore été activé. Veuillez payer au moment du ramassage.",
    sendSMS: "Recevoir des notifications SMS",
    history: "Historique de Commande",
    noHistory: "No Order History",
    timeRequested: "Heure de ramassage demandée",
    timeToPickup: "Temps de ramassage estimé",
    deliveryTimeRequested: "Requested Delivery Time",
    deliveryTimeToPickup: "Estimated Delivery Time",
    notAvailable:
      "Ce restaurant n'est pas disponible pour le moment. Veuillez réessayer plus tard",
    orderNotice: "Avis de la commande",
    orderMessage: "Message to the restaurant",
    enterMessage: "You can send a message to the restaurant.",
    datePlaced: "Time Placed",
    dateAccepted: "Time Accepted",
    dateEstimated: "Time Estimated",
    dateConfirmed: "Time Confirmed",
    dateCompleted: "Time Completed",
    statusName: "Status",
    phoneNumber: "Phone Numbers",
    totalCount: "Items",
    userName: "Nom d'utilisateur",
    customerInfo: "Your Info",
    itemName: "Élément de menu",
    options: "Option",
    memo: "Note",
    category1: "Catégorie 1",
    category2: "Catégorie 2",
    count: "Compter",
    unspecified: "(non spécifié)",
    adminQRCode: "Confirmation QR Code",
    paymentAndCancellation: "Paiement et annulation",
    statusTitle: "Order Status",
    sortOrder: "Sort Order",
    promotionNoteCard:
      "If you pay now by credit card, the discount will be applied.",
    promotionNoteStore:
      "If you pay later at the store, the discount will be applied.",
    sort: {
      newest: "Sort by newst order",
      oldest: "Sort by oldest order",
    },
    status: {
      unexpected: "Inattendue",
      all: "All",
      error: "Erreur",
      new_order: "Nouvelle Commande",
      validation_ok: "Valider",
      order_placed: "Commande éffectuer",
      order_accepted: "Accepter",
      cooking_completed: "Prêt à ramasser",
      ready_to_pickup: "Complete",
      transaction_complete: "Ramassage terminé",
      ready_to_shipping: "Ready to Shopping",
      waiting_payment: "Waigint payment",
      shipping_complete: "Shipping complete",
      transaction_hide: "Transaction terminée",
      order_canceled: "Annulé",
      order_refunded: "Remboursé",
      stripe_pending: "Paiement en attente",
      stripe_confirmed: "Paiement confirmé",
      stripe_canceled: "Paiement annulé",
      onsitePayment: "Paiement sur place",
      stripe_user_pending: "Card payment not completed",
      stripe_user_message_pending:
        "(The card payment process has not been completed on the restaurant side)",
      stripe_user_confirmed: "Card payment completed",
      stripe_user_message_confirmed: "I have already paid by credit card.",
      stripe_user_canceled: "Payment cancel",
      stripe_user_message_canceled: "Card payment has been cancelled",
      stripe_user_message_just_payment_canceled: "Please pay in the restaurant",
    },
    receipt: {
      receipt: "Receipt",
      loading: "Loading...",
      getReceipt: "View receipt",
      explain1: "You can view the receipt of Stripe's credit card payment",
      explain2: "A new tab will open. If you can't open it, please allow it",
    },
    onlinePaymentStatus: "Status of the card payment",
    lineLink: "Restaurant's LINE Official Account URL",
    lineMessage:
      "If you add LINE Official Account as a friend, you will receive information directly from the restaurant.",
    customerOrderHistory: "Order history",
    orderTimes: "Order",
    orderTimesUnit: "{count} times",
    cancelTimes: "Cancel",
    cancelTimesUnit: "{count} times",
    yourInfo: "Your information",
    continuousOrder: "Warning: a continuous order",
    lastOrder: "Last order",
    thisOrder: "This order",
    shippingCost: "Shipping Cost",
    isDelivery: "Delivery",
    ec: {
      customerInfo: "Customer info",
      formtitle: "Shipping information",
      zip: "Zip code",
      prefecture: "Prefecture",
      address: "Address",
      addressNotice: "Please enter all address information",
      name: "Full name",
      searchAddressFromZip: "Search for an address by zip code",
      select: "Select",
      email: "Email",
      phone: "Phone",
    },
    discount: "Discount",
    discountString: "Discount",
    discountAlert: {
      stripe:
        "If you pay now by credit card, the discount below will be applied.",
      instore:
        "If you pay later at the store, the discount below will be applied.",
    },
    discountPriceSuffix: "Discount",
    discountPriceMessage: "値引きがあります",
    discountPrice: "Discount Price",
    beforeDiscountPrice: "Before Discount Price",
    cancelReason: "Cancel Reason",
    productId: "Product Id",
    category: "Category",
    categoryId: "Category Id",
    subCategory: "SubCategory",
    subCategoryId: "SubCategory Id",
  },
  editEC: {
    postageList: "Shipping list",
    freeThreshold: "Set free shipping",
    copy: "Copy shipping above",
    setPostageFreeThreshold: "Set the amount of free shipping",
  },
  delivery: {
    alert:
      "Before setting the delivery, first specify the location of the store on the map in the store settings",
    areaSetting: "Delivery area setting",
    deliveryRange: "Delivery range radius",
    updateDeliveryRange: "Update range",
    areaTextExample:
      "Kita-Aoyama 1-chome to 3-chome and Minami-Aoyama 1-chome to 7-chome, Minato City",
    enableDelivery: "Enable delivery",
    deliveryLocation: "Delivery destination",
    deliveryFeeSetting: "Delivery Fee Settings",
    deliveryFee: "Delivery Fee",
    deliveryFreeThreshold: "Delivery Fee Free Settings",
    deliveryThreshold: "Delivery can be accepted Total amount setting",
    setAreaMap: "Set on the map",
    setAreaText: "Set on the text",
    setDeliveryLocation: "Set delivery destination",
    estimatedDistance: "Estimated distance",
    preparationTimeNotice:
      "Please enter the shortest time it will take for the customer to complete the delivery after placing the order.",
    setTheAddressInTheDeliveryLocation:
      "Set  the address in the delivery destination",
    setTheAddressInTheDeliveryLocationDescription:
      "* Calculate the distance between the store and the delivery destination.",
    availableDeliveryDistance: "Available Delivery Distance",
    reference: "(Reference)",
    aboutDeliveryArea: "Aboout Delivery Area",
    yen: "Yen",
    deliveryFreeThresholdNotice:
      "If the total price of the item exceeds this price, the delivery fee will be automatically set to 0 yen.",
    deliveryFeeSettingNotice:
      "If you do not need a delivery fee, please set it to 0 yen.",
    deliveryThresholdNotice:
      "If the total price of the item exceeds this price, we will be able to accept delivery.",
    setAreaMapNotice:
      "Specify the approximate deliverable range in a circle centered on the store. This range is a guide, so please judge whether delivery is possible at the time of ordering and accept the order.",
  },
  button: {
    adminTop: "Top Administrateur",
    back: "Retour",
    backToOrderListPage: "Voir les commandes",
    backToOrderPage: "View Order",
    backToDiscounts: "Voir les promotions",
    backToDocument: "View Documents",
    backToSubaccounts: "View Subaccounts",
    backToSmaregi: "View Smaregi",
    addTitle: "Ajouter un titre de groupe",
    addItem: "Ajouter un article",
    downloadMenu: "Télécharger le menu",
    cancel: "Annuler",
    next: "Suivant",
    category: "Catégorie",
    myPage: "My Page",
    backToTop: "Top",
    login: "Sign in",
    linkToAdminDocs: "Instructions détaillées",
    news: "Actualités",
  },
  sms: {
    signin: "Se connecter avec un numéro de téléphone",
    phonenumber: "Numéro de téléphone",
    pleasetype: "90-11-11-11",
    notice: "Un message SMS sera envoyé pour la vérification.",
    send: "Envoyer SMS",
    resend: "Renvoyer SMS",
    verificationCode: "Code de Verification",
    typeVerificationCode: "Veuillez saisir le code que vous avez reçu",
    userName: "Nom d'utilisateur (facultatif)",
    typeUserName: "John Smith",
    sendVerificationCode: "Envoyer",
    countryCode: "Indicatif Pays",
    country: {
      US: "United States (+1)",
      UK: "Royaume-Uni (+44)",
      FR: "France (+33)",
      BF: "Burkina Faso (+235)",
      TG: "TOGO (+228)",
      BN: "Bénin (+229)",
      CI: "Cote d'ivoire (+225)",
      SE: "Senegal (+335)",
      ES: "Espagne (+34)",
      DE: "Allemagne (+49)",
      JP: "Japon (+81)",
    },
    invalidPhoneNumber: "Numéro de téléphone invalide",
    invalidValidationCode: "Code de verification invalide",
    "auth/too-many-requests": "Too many requests",
    "auth/invalid-phone-number": "Numéro de téléphone invalide",
    "auth/invalid-verification-code":
      "Le code de vérification par SMS utilisé pour créer les informations d'identification d'authentification du téléphone n'est pas valide. Veuillez renvoyer le code de vérification par SMS et assurez-vous d'utiliser le code de vérification fourni par l'utilisateur.",
    "auth/missing-verification-code":
      "Les informations d'identification d'authentification du téléphone ont été créées avec un code de vérification SMS vide.",
    "auth/code-expired":
      "The SMS verification code has expired. Please resend the SMS verification code and enter it again",
  },
  admin: {
    welcomeMessage: "Bienvenue !",
    userManual: "Manuel de l'Utilisateur",
    suportPage: "Aide",
    facebookUserGroup: "Groupe",
    news: {
      title: "Actualité",
      newsTop: "Top Actualité",
      adminTop: "Top Administrateur",
    },
    yes: "Oui",
    yesOnce: "Oui(une fois)",
    no: "Non",
    promotion: {
      amount: "Montant",
      ratio: "Taux d'actualisation",
      discount: "Remise",
      onetimeCoupon: "Coupons émis une seule fois",
      multipletimesCoupon: "Coupons émis avec un nombre illimité",
      list: "Promotions",
      activationState: "Actuellement active",
      name: "Nom",
      activation: "Activer",
      type: "Type de remise",
      period: "Période d'application",
      minimumAmount: "Montant minimum disponible",
      yen: "yen",
      limitation: "Limite de l'utilisation",
      discounts: "Remise",
      paymentMethod: "Accepter",
      creditCard: "que les cartes de crédit",
      store: "que les paiements sur place",
      unrestricted: "les cartes de crédit et les paiements sur place",
      history: "Histoire",
      noHistory: "Non Historique des commandes",
      delete: "Supprimer",
      uid: "uid",
      rid: "rid",
      pid: "pid",
      oid: "oid",
      date: "Date",
      total: "Total",
      discountPrice: "Prix discount",
    },
    open: "Open",
    closed: "Closed",
    notes: {
      title: "Notes",
      userRestaurantsTitle:
        "À propos de la liste des restaurants pour l'utilisateur",
      userRestaurantsBody:
        "Les restaurants que vous avez ajoutés n'apparaîtront pas automatiquement dans la liste des restaurants des utilisateurs. Veuillez cliquer sur le bouton [Demande d'inscription] et votre restaurant sera ajouté à la liste après avoir été approuvé.",
      notificationSoundTitle: "À propos du son de notification",
      notificationSoundBody:
        "Un son de notification de commande sera joué lors de l'affichage de la page de la liste des commandes (pas sur cette page). Si vous souhaitez que plusieurs notifications de magasin soient lues en même temps, vous devez pouvoir afficher les pages de la liste des commandes pour chacune. Pour ce faire, vous aurez besoin d'une fenêtre distincte sur votre ordinateur et d'un appareil séparé pour votre tablette.",
    },
    hidePayment:
      "La fonction de paiement arrive bientôt. S'il vous plaît restez à l'écoute.",
    encourageToReadManual: {
      before:
        "Si vous n'êtes pas encore inscrit sur OwnPlate.com, veuillez suivre",
      manualName: '"ce guide d\'utilisation"',
      after: "et vous inscrire d'abord avant de poursuivre.",
    },
    forSignup: "Pour les restaurants qui ne se sont pas encore inscrits",
    pleaseSignIn: "Se Connecter (pour les restaurants)",
    goToSignIn: "Go Sign In",
    mailIsExpired: "Email is expired",
    passwordUpdated: "Password was updated",
    reSend: "Reset passwpord",
    registration: "S'Enregistrer (pour les restaurants)",
    email: "Email",
    password: "Mot de passe",
    confirmPassword: "Confirmer Mot de passe",
    name: "Nom & Prénom",
    emailPlaceHolder: "Enter email ici",
    passwordPlaceHolder: "Enter mot de passe ici",
    confirmPasswordPlaceHolder: "Retaper même mot de passe ici",
    enterName: "Enter nom ici",
    restaurant: "Restaurant",
    payment: "Methode de Payment",
    yourRestaurants: "Votre (vos) Restaurant (s)",
    noRestaurant: "Aucun restaurant",
    addYourRestaurant: "Veuillez ajouter votre Restaurant",
    addNewRestaurant: "Ajouter Nouveau Restaurant",
    noPaymentAccount: "Aucun compte de paiement",
    payments: {
      unsetWarning:
        "Vous devrez configurer un mode de paiement pour accepter les commandes.",
      onlinePayment: "Paiement en ligne",
      statusNotConnected: "Non Connecté",
      statusConnected: "Connecté",
      pleaseConnect:
        "Pour recevoir des paiements en ligne, vous devez connecter votre compte Stripe.",
      connectStripe: "Connectez votre compte Stripe",
      disconnectStripe: "Déconnecter le compte Stripe",
      reallyDisconnectStripe:
        "Voulez-vous vraiment déconnecter le compte Stripe?",
      openDashboard: "Ouvrir le tableau de bord Stripe",
      onsitePayment: "Paiement à la Livraison",
      pleaseCheck:
        "Veuillez cocher la case ci-dessous pour autoriser le Paiement à la Livraison.",
      onsitePaymentNote:
        "Veuillez être conscient des paiements non recouvrés en raison d'annulations soudaines ou de méfaits.",
      enableOnsitePayment: "Autoriser le Paiement à la Livraison",
    },
    mail: {
      magazine: {
        title: "Bulletins d'information par courrier électronique",
        body: "Recevez des courriels sur les actualités, les conseils et les rubriques.",
        optout: "Ne pas recevoir",
      },
    },
    thirdPartyService: {
      title: "Third Party Service",
      openSmaregi: "Open Smaregi",
      smaregiManual: "Smaregi Manual",
      smaregiAppMarket: "Go to Smaregi App Market",
      smaregiManualLink: "How to use Smaregi",
    },
    needEmailVerification:
      "Please complete the verification of your e-mail address.",
    noteEmailVerification1:
      "Please check the email sent from {email} and click on the URL in the email to complete the verification process.",
    noteEmailVerification2:
      "If you have not yet received the verification email, please check your spam folder or filter settings and resend a verification code.",
    sendVerificationCode: "Send verification code",
    verificationCodeSent: "We sent email",
    verificationCodeError: "Failed. Try again later.",
    viewAllOrders: "Voir toutes les commandes de tous les restaurants",
    viewPage: "Voir page",
    viewOrders: "Voir les commandes",
    incompleteOrders: "{count} incomplète | {count} incomplètes",
    editMenuItems: "Menu",
    pleaseAddMenu: "Aucun menu ajouté",
    itemSoldOut: "Complet",
    itemSoldOutToday: "Sold Out Today",
    itemPublished: "Published",
    itemNotPublished: "Not Published",
    editAbout: "Modifier les détails du restaurant",
    privateMode: "Mode privé",
    pleaseChangePublic:
      "Veuillez passer en mode public après avoir saisi les coordonnées du restaurant.",
    directory: {
      status: "Annuaire des restaurants",
      requestWarning:
        "Please apply after setting up payment and opening the restaurant to the public.",
      requestingWarning:
        "Please set up your payment settings and store listing if you have not already done so.",
      notListed: "Non listé",
      requestList: "Demande d'inscription",
      waiting: "En attente d'approbation",
      cancelRequest: "Demande d'annulation",
      listed: "Listé",
      unlist: "Se désinscrire",
    },
    delete: "Suprimer cet Restaurant",
    shareRestaurant: "Partager",
    pleaseSignUp: "S'inscrire en tant que nouvel utilisateur",
    forgotPassword: "Mot de passe oublié？",
    passwordReset: "Réinitialiser le mot de passe",
    pleaseCheckInbox:
      "Veuillez vérifier votre boîte de réception, un e-mail est en route.",
    order: {
      suspend: "Suspendre",
      suspendSettings: "Paramètres de suspension",
      suspendUntil: "Suspendre jusqu'à {display}",
      suspendNewOrders: "Suspendre les nouvelles commandes",
      suspendForAllDay: "Pour toute la journée",
      suspending: "Suspendre la liaison",
      unsuspendAt: "Réactiver à ",
      unsuspend: "Réactiver",
      suspendIndividualItem: "Articles Individuel",
      suspendAllItems: "Suspendre tout les articles",
      unsuspendAllItems: "Réactiver tout les articles",
      notification: "Notification",
      incompleteOrders: "Commandes incomplètes",
      notificationSettings: "Paramètres de Notification",
      soundOn: "Son de notification: Activé",
      soundOff: "Son de notification: Désactivé",
      infinityNotificationOn: "Rappelez chaque minute",
      infinityNotificationOff: "Notifier une fois",
      soundTest: "Tester",
      paymentIsNotCompleted:
        "Note : Veuillez compléter la procédure de paiement par carte de crédit en cliquant sur le bouton 'Complete' avant de remettre les articles au client.",
      cancelButton: "Annuler cette commande",
      cancelTitle: "Annulation de commande",
      cancelMessage:
        "Veuillez contacter le client et lui dire que la commande sera annulée.",
      paymentCancelButton: "Cancel only card payment",
      delete: "Annuler cette commande",
      deleteConfirm: "(Cette action est annulable)",
      paymentCancelTitle: "Cancel only card payment",
      paymentCancelMessage:
        "It will be changed to payment in the restaurant. Please contact the customer and let them know that you will be paying in the restaurant. The order will not be cancelled.",
      paymentCancel: "Cancel only card payment",
      paymentCancelConfirm: "(This action is undoable)",
      placedDate: "Date of order",
      pickupDate: "Date of Pickup",
      line: "Notification LINE",
      lineAdd: "Ajouter un compte LINE",
      lineUsers: "Comptes LINE Connectés",
      lineDelete: "Voulez-vous vraiment supprimer ce compte？",
      lineSafariPrivate:
        "Remarque: si vous utilisez le mode de navigation privée de Safari, vous ne pourrez pas vous connecter à LINE.",
      history: "Tout l'historique de l'ordre",
      more: "En savoir plus",
      all: "View All",
      deliveryOrder: "Delivery order",
      pickupOrder: "Pickup order",
      messageFromCustomer: "Message from the customer",
      changeOrderDetail: "Change order details",
      alreadyChanged: "already changed",
      willOrderChange: "Change order",
      cancelOrderChange: "Cancel an order change",
      confirmOrderChange: "Change order",
      cardPaymentMessage:
        "Payment has been completed. No need to receive payment at the store.",
      storePaymentMessage:
        "Payment has not been completed. Please receive payment at the store.",
      cardPayment: "Credit Card Payment",
      storePayment: "Store Payment",
    },
    menu: {
      delivery: "Delivery",
      printer: "Auto Print",
      discount: "Discount",
    },
    sound: {
      default: "Défault",
      gong: "Gong",
      magic: "Magic",
      doorChime: "Door Chime",
      coin: "Coin",
      cheepCheep: "Cheep Cheep",
      meow: "Meow",
    },
    qrcode: {
      title: "QRcodes",
      restaurant: "Page du Menu",
      download: "Télécharger QR Code",
      enter: "Enregistrer l'entrée",
      leave: "Enregistrer une sortie",
    },
    report: {
      title: "Rapport mensuel",
      "download-csv": "Télécharger le fichier Excel (Gains)",
      "download-csv-details": "Télécharger le fichier Excel (Détails)",
      "download-all-csv": "Download Excel File (Earnings)",
      "download-csv-history": "Download Excel File (History Earnings)",
      "download-csv-history-details": "Download Excel File (History Details)",
      "download-csv-monthly-details": "Download Excel File (Monthly Details)",
    },
    error: {
      memoLength: "Please enter your message within 500 characters.",
      email: {
        invalid: "Adresse e-mail invalide",
        taken: "L'adresse e-mail est déjà utilisée par un autre compte.",
      },
      password: {
        tooshort: "Le mot de passe doit contenir au moins 8 caractères",
        invalid: "Please enter alphabets and numbers for the password.",
        mismatch: "Les mots de passe ne corespondent pas",
      },
      code: {
        "auth/user-not-found":
          "Il n'y a pas d'enregistrement d'utilisateur correspondant à cet identifiant.",
        "auth/wrong-password": "Le mot de passe est invalide",
        "auth/too-many-requests":
          "Trop de tentatives de connexion infructueuses. Veuillez réessayer plus tard.",
        "auth/invalid-email": "L'adresse e-mail est mal formatée.",
        "auth/invalid-login-credentials":
          "The email or the password is invalid.",
        "auth/unauthorized-continue-uri":
          "Domaine non ajouté à la liste blanche du projet",
        "auth/internal-error": "Password is invalid.",
      },
    },
    index: {
      showSimple: "Simple",
      showAll: "All",
    },
    messages: {
      title: "Message",
      childInvitationAccept: "Accept the invitation and become a Sub-account",
      childInvitationAcceptMessage:
        "If you become a Sub-account, you will not be able to operate the restaurants added with this account.",
      childInvitationDeny: "Reject invitation",
      accept: "Accept",
      deny: "Reject",
      childInvitationMessage1:
        "I received an invitation from the owner who manages the following restaurants",
      childInvitationMessage2: "This restaurants",
      childInvitationMessage3:
        "If you want to change to a sub-account that manages this store, press Accept. If you want to decline, press Reject.",
    },
    subAccounts: {
      index: "Sub-account management",
      openDashboard: "Open Sub-account management",
      manualLink: "How to use sub-account",
      description:
        "You can set / manage sub-accounts that can only be operated at specific stores.",
      accountNotice:
        "The sub-account to be invited must be registered as a user.",
      title: "Sub-account management",
      subaccountlist: "Sub-account list",
      linkedStore: "Magasin Lié",
      NumberOfLinkedStores: "Nombre de Magasins Liés",
      status: "Statut",
      email: "Email",
      name: "Sub-account name",
      invite: "Invite",
      invitedList: "Invited List",
      deleteSubaccount: "Delete Sub-account",
      guidance:
        "Pour définir un magasin comme sous-compte, cliquez sur le nom du sous-compte.",
      send: "Send",
      sending: "Sending",
      enterName: "Please enter the name you want to give to the sub-account",
      enterEmail:
        "Please enter the email address of the sub-account you are inviting",
      inviteInputError: "Please check the input items",
      messageResult: {
        waiting: "Sent and waiting",
        accepted: "Accepted",
        denied: "Denied",
      },
      numberOfShops: "shops",
      selectRestaurant: "Please select a store that can operate this account.",
      confirmDeletechild: "Cancel the sub-account",
    },
    line: {
      title: "LINE連携",
      settings: "LINEチャンネル設定",
      users: "LINEユーザ一覧",
      notice1:
        "お店のLINE公式アカウントとおもちかえり.comを連携させて、お店のLINE公式アカウントのトークでお客様に注文状況をお知らせすることができます。",
      notice2:
        "ご利用にはLINE開発者サイトで、LINEログインチャンネルとMessaging APIの設定が必要です。",
      notice3: "Messaging APIはLINE公式アカウントと同じものを使います。",
      notice4:
        "LINEログインチャンネルにMessaging APIを「リンクされたボット」として登録しておくことが必要です。",
      notice5:
        "また、LINEログインの「コールバックURL」におもちかえりで指定するURLの登録が必要です。",
      loginClientId: "LINEログイン:チャンネルID",
      loginChannelSecret: "LINEログイン:チャンネルシークレット",
      messagingAccessToken: "Messaging API: チャンネルアクセストークン",
      callbackUrl: "LINEログイン:コールバックURL",
      showBelow: "設定の下のほうにあります",
      enabled: "有効にする",
      callbackURLCopyToLine: "この値をコピーしてLINE側に設定をしてください。",
    },
    smaregi: {
      index: "Smaregi",
      authenticating: "Authenticating",
      authenticationError: "Faild Authentication",
      smaregiShopList: "Smaregi store list",
    },
  },
  week: {
    short: {
      monday: "Lun",
      tuesday: "Mar",
      wednesday: "Mer",
      thursday: "Jeu",
      friday: "Ven",
      saturday: "Sam",
      sunday: "Dim",
    },
    shortest: {
      monday: "Lun",
      tuesday: "Mar",
      wednesday: "Mer",
      thursday: "Jeu",
      friday: "Ven",
      saturday: "Sam",
      sunday: "Dim",
    },
  },
  allergens: {
    title: "Les allergènes",
    shrimp: "Crevette",
    crab: "Crabe",
    gluten: "Gluten",
    egg: "Oeuf",
    milk: "Lait",
    soba: "Soba",
    peanuts: "Cacahuètes",
    fish: "Poisson",
    soybeans: "Soja",
    shellfish: "Fruits de mer",
    raw: "Brut",
    crustacean: "Crustacée",
    lupin: "Lupin",
    mollusc: "Mollusque",
  },
  line: {
    registerAsAFriend: "Please register as a friend",
    authenticating: "Authentication LINE",
    notifyMe: "Recevoir notification LINE",
    notifyMeFromFriend: "Receive LINE from Restaurant",
    addFriend: "Suivre",
    addFriendDescription:
      "Pour recevoir des notifications LINE, vous devez suivre le canal officiel",
  },
  trace: {
    thankyou: "Merci d'avoir participé.",
    enter: "Enter",
    leave: "Sortir",
    viewList: "Afficher la liste des traces",
    list: "Liste des traces",
  },
  errorPage: {
    popup: {
      title: "Une erreur inattendue s'est produite",
    },
    message: {
      generic: "Veuillez essayer cette opération plus tard.",
      line: "Veuillez réessayer de connecter LINE.",
      cardError:
        "If the error occurs repeatedly, please cancel the card payment only and process the order.",
    },
    code: {
      line: {
        validation: "L'authentification LINE a échoué.",
      },
      order: {
        checkout: "Échec de l'exécution du processus de paiement.",
        place: "Échec de la passation de la commande.",
        update: "Échec de la mise à jour de l'état de la commande.",
        updateCard: "Card processing failed.",
        cancel: "Échec de l'annulation de la commande.",
      },
      stripe: {
        confirm: "Échec de la confirmation du paiement.",
        confirm2:
          "If you continue to fail, please cancel the card payment and change to pay in the restaurant.",
        cancel: "Failed to cancel card payment",
        connect: "Échec de la connexion du compte Stripe.",
        disconnect: "Échec de la déconnexion du compte Stripe.",
        intent: "Échec du traitement du paiement.",
        NoJCB: "Ce restaurant n'accepte pas la carte JCB.",
      },
      smallPayment: "Card payments under 50 yen cannot be accepted.",
      menu: {
        save: "Échec de la mise à jour du menu.",
      },
      restaurant: {
        save: "Échec de la mise à jour du restaurant.",
      },
    },
    notFound: {
      message1: "La page que vous recherchez est introuvable.",
      message2: "Essayez de contacter le restaurant d'une autre manière.",
    },
    requireLogin: {
      message1: "Vous devez être connecté pour voir votre commande",
    },
  },
  dialogTips: {
    paymentMethods: {
      title: "Payment Methods",
      body: "This function provides information on payment methods available at the store for customers who have selected 'On Site Payment'",
    },
    phonenumber: {
      title: "Phone Number",
      body: "Displayed as a phone number for customers on the restaurant's page, and also notified by calling this number if phone notifications are turned on",
    },
  },
  printer: {
    description:
      "Cette fonctionnalité imprime automatiquement les commandes sur une imprimante de reçus Star Micronics lorsqu'une commande est reçue.",
    serverURL: "CloudPRNT URL du Serveur",
    guidance: "Veuillez configurer cette URL sur votre imprimante.",
    reset: "Réinitialiser l'URL du Serveur",
  },
  validationError: {
    zip: {
      empty: "Please enter your zip code",
      invalidZip: "Please enter your valid zip code",
    },
    address: {
      empty: "Please enter your shipping address",
    },
    name: {
      empty: "Please enter your full name",
    },
    memo: {
      length: "Please enter your message within 500 characters.",
    },
    location: {
      noLocation:
        "After entering the address, please update the delivery distination on the map. You can also update by tapping ",
    },
    email: {
      empty: "Please enter your email address",
      invalidEmail: "Please enter your valid email address",
    },
  },
};

export default data;
