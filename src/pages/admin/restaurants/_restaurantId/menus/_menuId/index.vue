...
<div>
  ...
  <template v-else>
    <section class="section">
        ...
        <div class="column is-6">
          ...
          <b-field :type="errors['tax'].length > 0 ? 'is-danger' : 'is-success'" >
            <b-select v-model="menuInfo.tax" placeholder="select">
              <option v-for="taxItem in taxRates" :key="taxItem" :value={taxItem}>
                {{ taxItem }} {{ restaurantInfo && ((restaurantInfo[taxItem + "Tax"] || 0) + "%") }}
              </option>
            </b-select>
          </b-field>
        </div>
      ...
      <b-checkbox v-model="menuInfo.publicFlag" :disabled="hasError">
        {{$t('shopInfo.public')}}
      </b-checkbox>
...
export default {
  ...
  async created() {
    ...
    const menuRes = db.doc(`restaurants/${this.restaurantId()}/menus/${this.menuId}`);
