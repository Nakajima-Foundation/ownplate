<template>
  <!-- for Promotion -->
  <div>
	<div class="mb-2 border-4 border-op-teal text-op-teal text-center font-bold fixed left-4 right-4 mx-auto max-w-lg cursor-pointer items-center rounded-full bg-white p-3 shadow-lg bottom-3 z-30 sm:bottom-8" @click="promotionVisible=true">
		<div class="text-xs">
  ...
  <o-modal :active.sync="promotionVisible" width="80%" scroll="keep">
    <div class="my-6 rounded-lg bg-white shadow-lg border-op-teal border-2">
      ...
      <div v-for="(v, k) in possiblePromotions" :key="k" class="px-6">
        <div class="mt-5 text-lg font-bold text-opacity-40">{{ v.promotionName }}</div>

				<div class="font-bold mt-2 text-op-teal">
      		<template v-if="v.discountMethod === 'amount'">{{ $t("promotion.fromAmountSpent") }}
						<span class="text-xl">¥{{ v.discountValue }}</span>
						{{ $t("promotion.discount") }}</template>
      		<template v-if="v.discountMethod === 'ratio'">{{ $t("promotion.fromAmountSpent") }}
						<span class="text-xl">{{ v.discountValue }}%</span>
						{{ $t("promotion.discount") }}</template>
				</div>
        ...
        <div v-if="v.hasTerm">
      	  ...
      	  {{ $t("promotion.terms") }}
				...
				<li class="ml-5">
				  {{ v.usageRestrictions  ? $t("promotion.oneTimeOnly") : $t("promotion.noLimit")}}
				</li>
				</ul>
