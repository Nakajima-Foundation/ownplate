<template>
  <div>
    <div class="flex space-x-4">
			<!--ToDo OrderListPage（現在の遷移先と同じ）に戻れるようにする-->
      <div
        class=" flex-shrink-0 inline-flex h-9 items-center justify-center rounded-full bg-black bg-opacity-5 px-4"
      >
        <i class="material-icons mr-2 text-lg text-op-teal">arrow_back</i>
        <span class="text-sm font-bold text-op-teal">{{
          $t("button.backToOrderListPage")
        }}</span>
      </div>
