...
- [x] 管理者がワンタイム URL を作れる
- [x] 非ログインの端末がその URL から登録できる
- [x] 実際の注文で通知が届く（macOS Chrome / モバイル）

未確認:

- [ ] URL が一度しか使えないこと、期限切れが弾かれること
- [ ] `pushInvites` がクライアントから read も list もできないこと
- [ ] 一覧の ON/OFF と削除
- [ ] `notify` を OFF にした端末に届かないこと
- [ ] iOS でホーム画面に追加してから登録できること（Android で確認したのか iOS なのかが未確定）
- [ ] 注文者の店舗ページに Service Worker も manifest も入らないこと
- [ ] 登録が無い店舗でも、既存の LINE / メール / 電話 / 通知音 が従来どおり動くこと
