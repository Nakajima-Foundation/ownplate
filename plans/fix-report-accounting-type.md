...
素の場合・`accounting` に setter がある場合・**`accounting` が読むたびに変わる場合**の
...
**差ゼロではない。** 採用した形の差は **すべて「`order.accounting` が読むたびに変わる」
一形だけ**で、内訳は「旧は返す／新は落ちる」。足した例外が発火するのはそこだけで、
それ以外のすべての形では差が出ない。Firestore の `doc.data()` は素のオブジェクトを
返すので、この形は実データでは作れない。
...
- 守り単体が、有る／無いの両方で正しく答えること

見送った2つの形に差し替えると順に2件・1件が赤くなる。**守りだけを外すと、
守りの2件だけが赤くなる。** 戻すと全部緑に戻る。
