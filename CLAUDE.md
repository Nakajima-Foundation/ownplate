...
OwnPlate は飲食店向けのテイクアウト注文サービス。元々は海外展開も視野に作られたが、現在は国内向けに運用している。Firebase + Vue + Stripe 構成で、UberEats 等の中間マージンを取らず Stripe の決済手数料のみで回すのが設計方針。
...
- バックエンド: Firebase Functions (Node.js 24) / Express / Firestore
- 決済: Stripe
- 監視: Sentry（フロント・バックエンド両方）
- 通知: Twilio (SMS) / Nodemailer (メール) / LINE (LIFF)
- E2E: Playwright
- Functions のユニットテスト: `node --test`（tsx loader）と mocha が混在
...
- `functions/src/` — Firebase Functions。Express ベースの API とバックグラウンド処理。
...
`src/` と `functions/src/` で共有する TypeScript（モデル・定数・ユーティリティ・電話番号処理など）は、**import 共有ではなく `scripts/copy2functions.sh` によって物理コピー** される。コピー対象の例:

- `src/config/constant.ts` → `functions/src/common/constant.ts`
- `src/utils/phoneutil.ts` → `functions/src/common/phoneutil.ts`
- `src/utils/commonUtils.ts` → `functions/src/utils/commonUtils.ts`
- `src/models/{RestaurantInfo,menu,customer,orderInfoData,functionTypes,orderTypes,promotionTypes}.ts` → `functions/src/models/`

このコピーは `firebase.json` の `functions.predeploy` フックで自動実行される。**Claude Code が共通ロジックを編集するときは `src/` 側を直すのが原則**。`functions/src/common/` や `functions/src/models/` を直接編集しても、次回デプロイ時にコピーで上書きされる。

例外: `src/utils/common.ts` はコピー対象外（Vue 用と Functions 用で import パスが異なるため、両方に個別実装がある）。
