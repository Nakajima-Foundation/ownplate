...
**いまは保留（2026-09-22）。** 「注文者への LINE 通知を push に置き換える」案は、
置き換えでは downgrade になると判断して止めた。理由と、やるとしたら何から測るかは
issue #1810 を参照。作り始める前にそちらを読むこと。
...
- 一覧・ON/OFF・削除・名前の変更はクライアントから直接行う。ただし rules は `lines` と違い、
  **更新できるフィールドを `notify` と `name` だけに絞っている**。この doc はサーバだけが持つ値
  （`recentSends` の送信記録、`restaurantId`、登録時の情報）を抱えているため。
  禁止するフィールドを並べる形にしないのは、後から足したフィールドが素通りするから。
...
- 一覧は `recentSends` の直近ぶん（`FAILURE_ALERT_WINDOW`）を見て、**2種類に出し分ける**。
  「宛先そのものが死んでいる」失敗（`dead` が立つ）だけが再登録で直るので、そこだけ
  「登録し直してください」と出す。payload 不正や一時的な失敗には「送信に失敗しています」と
  コードを出す。まとめて再登録を指示すると、直らない作業をさせることになる。
- 記録は端末ごとに1件ずつトランザクションで書く。読んでから書くまでの間に別の注文の送信が
  挟まると記録が消え、消えるのが失敗の記録だとアラートが出ない。
