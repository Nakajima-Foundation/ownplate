...
export const invite = async (
  ...
  try {
    ...
    await db
      .doc(`/admins/${childUid}/messages/childInvitation${adminUid}`)
      .set(invitationData);
    return {
      result: true,
      childUid: childUid,
...
export const invitationValidateProcess = async (
  ...
  callback: (
    adminUid: string,
    messageData: firebase.firestore.DocumentData,
    messageRef: firebase.firestore.DocumentReference
  ) => Promise<void>
) => {
...
const childInvitationProcess = async (
  ...
  await invitationValidateProcess(
    db,
    data,
    context,
    async (
...
export const accept = async (
  ...
  try {
    await childInvitationProcess(
      db,
      data,
      context,
      async (messageData, messageRef) => {
        await db.runTransaction(async (tr) => {
          const customClaims = { parentUid: messageData.fromUid };
          await firebase
            .auth()
            .setCustomUserClaims(messageData.toUid, customClaims);

          const childRef = db.doc(
            `admins/${messageData.fromUid}/children/${messageData.toUid}`
          );
          ...
          if (
            ...
            throw new functions.https.HttpsError(
              "invalid-argument",
              `The child or message does not exist.`
            );
          }
          //  accepted
  ...
  try {
    await db.runTransaction(async (tr) => {
      ...
      const messageRef = db.doc(
        `/admins/${childUid}/messages/childInvitation${adminUid}`
      );
      const messageDoc = await tr.get(messageRef);
