...
const clientSecrets = (functions.config() && functions.config().smaregi && functions.config().smaregi.clientsecrets) || {[smaregi.clientId]: process.env.SmaregiClientSecret};
...
export const processAction = async (data) => {
  ...
  if (data.action === 'edited' && data.event === 'pos:stock') {
    ...
    data.ids.map((async (idData) => {
      ...
      const stockListData = await stockApi.list({store_id: storeId, product_id: productId});
