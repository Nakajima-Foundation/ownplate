...
const validated = (
  data: object,
  result: { result: boolean; errors: unknown[] },
  name: string,
) => {
  if (!result.result) {
...
export const registerWebPush = async (
  ...
  await registrationsCollection(db, uid)
    .doc(data.fid)
    .set(
      {
        fid: data.fid,
...
export const sendTestWebPush = async (
  ...
  const payload = createWebPushData(
    title,
    (data?.body ?? "").trim(),
    TEST_PUSH_PATH,
  );
