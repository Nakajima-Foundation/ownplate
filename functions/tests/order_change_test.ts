...
describe('Order function', () => {
  it ('Order function, orderCounter test', async function() {
    ...
    const placed = await order.place(adminDB, {
      restaurantId, orderId, timeToPickup: {seconds: 1613986197, nanoseconds: 0},
      tip: 100
    }, context );
    console.log(JSON.stringify(placed, undefined,4));

    const ownerUid = "121212";
    const ownerContext = {auth: { uid: ownerUid, token:{ phone_number: "xxxx"}}} as Context;
    const change = await intent.orderChange(adminDB, {
