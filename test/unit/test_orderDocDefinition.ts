...
import type { OrderInfoData, OrderItemData } from "../../src/models/orderInfoData.ts";
...
const texts = (doc: { content: unknown[] }): string => {
  const walk = (node: unknown): string => {
    ...
    if (node && typeof node === "object" && "text" in node) return walk(node.text);
    return "";
  };
...
describe("buildOrderDocDefinition — 登録番号", () => {
  it("prints the number when the restaurant has one", () => {
    assert.ok(build({ invoiceNumber: "T1234567890123" }).includes("T1234567890123"));
  });
  ...
  it("puts the number above the platform line", () => {
    ...
    assert.ok(printed.indexOf("登録番号") < printed.indexOf("おもちかえり.com"));
  });
});
...
describe("buildOrderDocDefinition — 税", () => {
  ...
  it("falls back to a single tax line for an order with no breakdown", () => {
    const printed = build({}, { accounting: undefined });
    assert.ok(printed.includes("内税額"));
  ...
  it("takes the tax mode from the order, not the restaurant's current setting", () => {
    assert.ok(build({ inclusiveTax: true }, { inclusiveTax: false }).includes("外税額"));
    assert.ok(build({ inclusiveTax: false }, { inclusiveTax: true }).includes("内税額"));
  });
});
...
describe("buildOrderDocDefinition — 明細と体裁", () => {
  ...
  it("says カード決済 when the order was paid by card", () => {
    assert.ok(build({}, { payment: { stripe: "pi_123" } }).includes("カード決済"));
